...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "Arial, sans-serif", padding: 24, background: "#f4f7fb", minHeight: "100vh" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", background: "#fff", padding: 24, borderRadius: 12, boxShadow: "0 6px 20px rgba(0,0,0,0.08)" }}>
        <h1 style={{ textAlign: "center", color: "#2c3e50", marginBottom: 8 }}>POLICE DASHBOARD</h1>
        <p style={{ textAlign: "center", color: "#555", marginTop: 0 }}>Live alerts and E-FIRs</p>
            ...
            <div>
              ...
              {alerts.length === 0 ? <p>No alerts yet.</p> : (
                <div style={{ display: "grid", gap: 10 }}>
                  {alerts.map((a) => (
                    <div key={a.id || Math.random()} style={{ padding: 12, borderRadius: 8, background: "#fff", border: "1px solid #e6eaf0", cursor: "pointer" }}
                         onClick={() => {
                        if (a.type === 'missing') {
                          alert(
                            `Alert: ${a.type}\n` +
          ...
          {view === "efirs" && (
            <div>
              ...
              {efirs.length === 0 ? <p>No E-FIRs generated yet.</p> : (
                <div style={{ display: "grid", gap: 10 }}>
                  {efirs.map((e) => (
                    <div key={e.id || Math.random()} style={{ padding: 12, borderRadius: 8, background: "#fff", border: "1px solid #e6eaf0", cursor: "pointer" }}
                         onClick={() => {
                          let msg = `E-FIR\nStation: ${e.station}\nDate: ${formatDate(e.dateTime)}\nDetails: ${e.details || ""}`;
                          
                          // Include missing-person info if present
                          if (e.missingName) {
